...
function train_loop(all_train, all_train_label, test, test_label, dev, dev_label, w2v)
  ...
  for fold = 1, opt.folds do
    ...
    dev_label = dev_label:narrow(1, 1, dev_size)
      --train = train:narrow(1, dev_size+1, train_size)
      --train_label = train_label:narrow(1, dev_size+1, train_size)
    train = train:narrow(1, 1, J)
    train_label = train_label:narrow(1, 1, J)      
      --local shuffle = torch.randperm(train_size):long()
    ...
    for epoch = 1, opt.num_epochs do
      ...
      -- Dev
      local dev_err, dev_predict, dev_target = trainer:test(dev, dev_label, model, criterion)
      ...
      local test_err, test_predict, test_target = trainer:test(test, test_label, model, criterion)
    ...
    if opt.train_only == 0 then
      local test_err, test_predict, test_target = trainer:test(test, test_label, best_model, criterion)
	  --torch.save('result/predict', test_predict)
	  --torch.save('result/true', test_target)
      print('test perf ', 100*test_err, '%')
